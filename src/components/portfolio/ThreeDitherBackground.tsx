...
export default function ThreeDitherBackground() {
  ...
  useEffect(() => {
    ...
    const handleMouse = (e: MouseEvent) => {
      ...
      gsap.to(auroraUniforms.uMouse.value, { x, y, duration: 2.5, ease: 'power2.out' })
    }
  ...
  useEffect(() => {
    ...
    const isDark = theme === 'dark'
    const colors = getThemeColors(isDark)
    const { auroraUniforms, particleUniforms, bloomPass, renderer } = sceneRef.current

    gsap.to(auroraUniforms.uColor1.value, { r: colors.color1.r, g: colors.color1.g, b: colors.color1.b, duration: 0.8 })
    gsap.to(auroraUniforms.uColor2.value, { r: colors.color2.r, g: colors.color2.g, b: colors.color2.b, duration: 0.8 })
    gsap.to(auroraUniforms.uColor3.value, { r: colors.color3.r, g: colors.color3.g, b: colors.color3.b, duration: 0.8 })
    gsap.to(auroraUniforms.uIntensity, { value: colors.intensity, duration: 0.8 })
    ...
    gsap.to(particleUniforms.uColor.value, { r: colors.particleColor.r, g: colors.particleColor.g, b: colors.particleColor.b, duration: 0.8 })
    renderer.toneMappingExposure = isDark ? 1.2 : 1.0
  }, [theme])
